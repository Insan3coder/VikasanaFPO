...
public class ProjectService extends BaseService {
	...
	private ProjectDto assignProjectToDto(Project project) {
		if (Objects.nonNull(project)) {
			...
			/*
			 * p.setEmployees(project.getEmployees().stream().map(x ->
			 * x.getEmployee().getEmployeeFirstName() + " " +
			 * x.getEmployee().getEmployeeLastName()) .collect(Collectors.toList()));
			 * p.setTechnologies(project.getTechnologies().stream().map(x ->
			 * x.getTechnology().getTechnologyName()).collect(Collectors.toList()));
			 */return p;
		} else {
			return null;
		}
